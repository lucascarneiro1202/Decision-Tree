...
class BaseDecisionTree:
    ...
    def _build_tree(self, X: pd.DataFrame, y: pd.Series, depth=0):
        ...
        if n_labels == 1:
            leaf_value = y.iloc[0]
            return Node(value=leaf_value, is_leaf=True, samples=n_samples, class_dist=class_dist, criterion=current_impurity)
        ...
        if "threshold" in best_split:
            ...
            return Node(feature=feature_name, threshold=threshold, branches=branches, majority_class=maj_class, samples=n_samples, class_dist=class_dist, criterion=current_impurity)
        ...
        # If the division is for a categorical feature with only two values (CART)  
            ...
            return Node(feature=feature_name, left_values=left_values, branches=branches, majority_class=maj_class, samples=n_samples, class_dist=class_dist, criterion=current_impurity)
        ...
        # If the division is for a categorical feature with multiple values (ID3 and C4.5)
            ...
            return Node(feature=feature_name, branches=branches, majority_class=maj_class, samples=n_samples, class_dist=class_dist, criterion=current_impurity)
